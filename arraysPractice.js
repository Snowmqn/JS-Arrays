...
function reversedLooper (letters) {
  for (var i=letters.length-1;i>=0;i--) alert(letters[i]);
}
...
function finder (numList) {
  var num = getRandomArbitrary();
  var result = false
  for (let i=0;i<numList.length;i++) {
    if (numList[i] === num) result = true;
  }
  return result;
}
...
function removeItem (myGroceryList, item) {
  ...
  if (typeof myGroceryList[0] !== 'string') return [];
  for (let i=0;i<myGroceryList.length;i++) {
    if (item === myGroceryList[i]) myGroceryList.splice(i,1);
  }
...
function addItem (myGroceryList, item) {
  if (typeof item !== 'string') return [];
  // if (typeof myGroceryList !== []) return [];
...
function maker () {
  var result = [];
  for (let i=1;i<=215;i++) result.push(i);
  return result;
}
...
function addTen(numbers) {
  for (let i=0;i<numbers.length;i++) numbers[i] = 10 + Number(numbers[i]);
  return numbers;
}
